...
// マクロ定義
//===============================================
#define SIZE_X			(30.0f)						// 横サイズ
#define SIZE_Y			(30.0f)						// 縦サイズ
...
HRESULT CUiTarget::Init(D3DXVECTOR3 pos, int nPriority)
{
	...
	{
		{ "data\\TEXTURE\\ui002.png" },	// 燃えるゴミ
		{ "data\\TEXTURE\\ui003.png" },	// 燃えないゴミ
		{ "data\\TEXTURE\\ui004.png" }	// プラスチックゴミ
	};

	D3DXVECTOR3 TargetPos = CGame::GetTarget()->GetPos();	// ターゲットの位置を取得
	float fPlayerRot = CGame::GetPlayer()->GetRot().y;		// プレイヤーの向きを取得
...
void CUiTarget::Update(void)
{
	...
	bool bKeyPlastic = false;		// プラスチックゴミのキー入力されたか
	...
	{// 右キーが押された
		...
		bKeyPlastic = true;		// プラスチックゴミのキー入力された
	}
