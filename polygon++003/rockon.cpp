...
{
	// 縮小処理
	CObjectBillboard::ReductionSize(DIFF_SIZE);

	// 捨てたときにロックオンを外す
	if ((m_separation == CGarbage::SEPARATION_BURN && (CManager::GetKeyboardInput()->GetRelease(DIK_LEFT) == true || CManager::GetInputGamePad()->GetRelease(CInputGamePad::BUTTON_X, 0) == true))
		|| (m_separation == CGarbage::SEPARATION_NONFLAMMABLE && (CManager::GetKeyboardInput()->GetRelease(DIK_UP) == true || CManager::GetInputGamePad()->GetRelease(CInputGamePad::BUTTON_Y, 0) == true))
		|| (m_separation == CGarbage::SEPARATION_PLASTIC && (CManager::GetKeyboardInput()->GetRelease(DIK_RIGHT) == true || CManager::GetInputGamePad()->GetRelease(CInputGamePad::BUTTON_B, 0) == true)))
	{// SPACEキーが押された
		// 終了処理
		Uninit();
	}
...
void CRockon::SetSeparation(CGarbage::SEPARATION separation)
{
	...
	case CGarbage::SEPARATION_NONFLAMMABLE:		// 燃えないゴミ
		// オブジェクトビルボードの色設定
		CObjectBillboard::SetCol(D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f));	// 緑
		break;

	case CGarbage::SEPARATION_PLASTIC:			// プラスチックゴミ
		// オブジェクトビルボードの色設定
		CObjectBillboard::SetCol(D3DXCOLOR(0.0f, 0.0f, 1.0f, 1.0f));	// 青
		break;
	}
}
