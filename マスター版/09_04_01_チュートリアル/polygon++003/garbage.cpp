...
void CGarbage::Update(void)
{
	...
	case STATE_INPLAYER:		// 描画しない
		...
		{
			if ((CManager::GetMode() == CScene::MODE_TUTORIAL &&
				CTutorial::GetDumpster(nCntDumpster)->GetPos().x <= m_pos.x + DUMPSTER_LENTH && CTutorial::GetDumpster(nCntDumpster)->GetPos().x >= m_pos.x - DUMPSTER_LENTH
				&& CTutorial::GetDumpster(nCntDumpster)->GetPos().z <= m_pos.z + DUMPSTER_LENTH && CTutorial::GetDumpster(nCntDumpster)->GetPos().z >= m_pos.z - DUMPSTER_LENTH)
				|| (CManager::GetMode() == CScene::MODE_GAME &&
				CGame::GetDumpster(nCntDumpster)->GetPos().x <= m_pos.x + DUMPSTER_LENTH && CGame::GetDumpster(nCntDumpster)->GetPos().x >= m_pos.x - DUMPSTER_LENTH
			...
			{// ターゲットがゴミステーションを利用できる範囲内に入った
				...
				{// SPACEキーが押された
					m_posDest.y = DIFF_HEIGHT;		// 目的の高さを設定
					m_nIdxDumpster = nCntDumpster;	// ゴミステーションの番号を保存
	...
	{// 描画しない状態じゃない
		...
		{// それ以外
			...
			case SEPARATION_BURN:			// 燃えるゴミ
				// オブジェクトXの描画処理（橙）
				CObjectX::DrawXFile(m_aIdxXFile[m_type], CXFile::COL_ORANGE);
				break;

			case SEPARATION_NONFLAMMABLE:	// 燃えないゴミ
				// オブジェクトXの描画処理（水色）
				CObjectX::DrawXFile(m_aIdxXFile[m_type], CXFile::COL_LIGHTBLUE);
				break;

			case SEPARATION_PLASTIC:		// プラスチックゴミ
				// オブジェクトXの描画処理（黄緑）
				CObjectX::DrawXFile(m_aIdxXFile[m_type], CXFile::COL_YELLOWGREEN);
				break;
